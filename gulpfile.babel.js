// Generated on 2017-04-05 using generator-angular-fullstack 3.8.0
'use strict';
  ...
  return gulp.src(paths.client.mainView)
    .pipe(plugins.inject(
      ...
      .pipe(plugins.sort(sortModulesFirst)), {
        ...
        endtag: '<!-- endinjector -->',
        transform: (filepath) => '<script src="' + filepath.replace(
          `/${clientPath}/`, '') + '"></script>'
      }))
  ...
  return gulp.src(paths.client.mainView)
    .pipe(plugins.inject(
      ...
      .pipe(plugins.sort()), {
        ...
        endtag: '<!-- endinjector -->',
        transform: (filepath) => '<link rel="stylesheet" href="' +
          filepath.replace(`/${clientPath}/`, '').replace('/.tmp/', '') +
          '">'
      }))
    .pipe(gulp.dest(clientPath));
});
...
gulp.task('lint:scripts', cb => runSequence(['lint:scripts:client',
  'lint:scripts:server'
], cb));
...
gulp.task('lint:scripts:server', () => {
  return gulp.src(_.union(paths.server.scripts, _.map(paths.server.test,
      blob => '!' + blob)))
...
gulp.task('watch', () => {
  var testFiles = _.union(paths.client.test, paths.server.test.unit, paths.server
    .test.integration);
...
gulp.task('serve', cb => {
  runSequence(['clean:tmp', 'constant', 'env:all'], ['lint:scripts',
      'inject'
    ], ['wiredep:client'], ['transpile:client', 'styles'], [
...
gulp.task('serve:debug', cb => {
  runSequence(['clean:tmp', 'constant'], ['lint:scripts', 'inject'], [
      'wiredep:client'
...
gulp.task('test:e2e', ['env:all', 'env:test', 'start:server',
  'webdriver_update'
], cb => {
